...
public class GameSceneController : MonoBehaviour
{
    ...
    //publiv event Action<int> LifeLost --> event with int action
    ...
    private void StartLevel(int levelIndex)
	{
        ...
        //StartCoroutine(SpawnShip(false)); --> spawn ship at the start of the game 
    ...
    private IEnumerator SpawnShip(bool delayed)
    {
        //if(delayed) yield return shipSpawnDelay;
        if(delayed)
            yield return shipSpawnDelay;
        //PlayerController ship = Instantiate(playerShip, new Vector2(0, 4.7f), Quartenion.identity) --> spawn ship
        ...
        //ship.HitByEnemey  += Ship_HitByEnemy --> subscribe to event with function
        yield return null;
    }
    ...
    private void Ship_HitByEnemy()
    {
        ...
        {
            LifeLost(lives);
            //invoke lifelost event --> HUD controller to handle event
        }
        if (lives > 0)
        {
    ...
    private IEnumerator SpawnPowerUp()
    {
        while (true)
        {
            ...
            //int index = UnityEngine.Random.Range(0, powerUpPrefabs.Length) --> random.range can be unityengine or systems so its explicitly declared here.
            Vector2 spawnPosition = ScreenBounds.RandomTopPosition();
